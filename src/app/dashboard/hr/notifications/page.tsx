    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Updates for HR workflow events, stage transitions, and automation actions.</p>
        </div>
      ...
      <Card className="rounded-2xl border-slate-200/80 bg-white/95 shadow-sm">
        <CardHeader className="space-y-3">
          <div className="flex flex-wrap gap-2">
            <Button type="button" size="sm" variant={mode === "all" ? "primary" : "outline"} className="rounded-full px-4" onClick={() => setMode("all")}>
              All
            </Button>
            <Button
              type="button"
          ...
          <CardDescription>Click any notification to open the related page and mark it read.</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <EmptyState
              title={emptyTitle}
              description="You will see HR notifications here when candidates move across rounds."
              icon={<BellRing size={18} />}
            />
          ) : (
            <div className="space-y-3">
              {items.map((n) => (
                <div key={n.id} className={`rounded-xl border p-4 transition ${n.is_read ? "border-slate-200 bg-white" : "border-indigo-100 bg-indigo-50/40"}`}>
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <Link href={n.route || "/dashboard/hr/candidates"}>
                        <Button
                          size="sm"
                          variant="outline"
